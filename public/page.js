const vm = new Vue ({
  ...
  async created () {
    this.addNotification('welcome to the chatapp, keypair will be geneated.')
    this.cryptWorker = new Worker('rsa.js')
  ...
  methods: {
    setupSocketListeners () {
      ...
      this.socket.on('NEW_CONNECTION', () => {
        this.addNotification('Another user has joined the chat.')
        this.sendPublicKey()
      })
      ...
      this.socket.on('ROOM_FULL', () => {
        this.addNotification(`Cannot join ${this.pendingRoom}, chat room is full`)
